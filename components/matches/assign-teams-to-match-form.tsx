...
export function AssignTeamsToMatchForm({
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      <div>
        <label className="text-xs font-medium text-muted-foreground block mb-1.5">Team 1</label>
        <select
          value={newHomeId}
          onChange={(e) => setNewHomeId(e.target.value)}
          className="w-full h-10 rounded-lg border border-border bg-card px-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-accent"
        >
      ...
      <div>
        <label className="text-xs font-medium text-muted-foreground block mb-1.5">Team 2</label>
        <select
          value={newAwayId}
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          className="flex-1 h-10 rounded-lg bg-accent text-accent-foreground text-sm font-medium hover:bg-accent/90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {loading ? (

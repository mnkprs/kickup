...
import { updateTeamAvatarUrlAction } from "@/app/actions/teams";
...
interface TeamAvatarUploadProps {
  ...
  /** When true, clicking triggers file picker for upload */
  editable?: boolean;
}
...
export function TeamAvatarUpload({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const path = `${team.id}/avatar.${ext}`;
      const { error: uploadError } = await supabase.storage
        .from("team_avatars")
        .upload(path, file, { upsert: true });
      if (uploadError) throw uploadError;

      const { data } = supabase.storage.from("team_avatars").getPublicUrl(path);
      const result = await updateTeamAvatarUrlAction(team.id, data.publicUrl);
  ...
  const avatarContent = (
    <div
      ...
      style={
        !team.avatar_url
          ? { backgroundColor: `${team.color ?? "#2E7D32"}33` }
          : undefined
      }
    >
      {team.avatar_url ? (

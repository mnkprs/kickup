...
import { Camera, ChevronDown, ChevronUp, MapPin, Pencil, Shield, ShieldCheck } from 'lucide-react';
...
export function PlayerProfile() {
  ...
  const { profile, user, isAdmin, captainTeam, playerTeams, refreshProfile } = useAuth();
  ...
  const playerMatches = useMemo(() =>
    matches.filter(m => m.status === 'completed' && playerMatchIds.has(m.id)).slice(0, 6),
  ...
  return (
    <div style={{ background: bg, minHeight: '100vh', fontFamily: 'Roboto, sans-serif' }}>
      {/* Hero Header */}
      <div className="relative" style={{ background: 'linear-gradient(135deg, #1B5E20 0%, #2E7D32 60%, #388E3C 100%)', paddingTop: '48px', paddingBottom: '36px' }}>
        ...
        <div className="flex flex-col items-center gap-3 px-4">
          ...
          <div className="relative">
            <div style={{ padding: '3px', borderRadius: '50%', background: 'linear-gradient(135deg, rgba(255,255,255,0.6), rgba(255,255,255,0.1))' }}>
              <PlayerAvatar
          ...
          <div className="text-center w-full">
            <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'white', letterSpacing: '-0.3px' }}>{profile?.full_name ?? 'Player'}</h1>

            {profile?.bio ? (
              <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.7)', marginTop: '4px', maxWidth: '280px', margin: '6px auto 0' }}>
                {profile.bio}
              </p>
            ) : null}
            ...
            <div className="flex items-center justify-center gap-2 mt-2 flex-wrap">
              {profile?.position ? (
                <span className="px-2.5 py-0.5 rounded-full" style={{ background: 'rgba(255,255,255,0.22)', color: 'white', fontSize: '12px', fontWeight: 700 }}>
                  {profile.position}
                </span>
              ) : null}
              {profile?.nationality ? (
                <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.75)' }}>{profile.nationality}</span>
              ) : null}
              {profile?.is_field_owner ? (
                <span className="px-2.5 py-0.5 rounded-full flex items-center gap-1" style={{ background: 'rgba(255,255,255,0.22)', color: 'white', fontSize: '12px', fontWeight: 600 }}>
                  🏟️ Field Owner
                </span>
              ) : null}
            </div>
            ...
            {(() => {
              ...
              return bio.length > 0 ? (
                <div className="flex mt-4 rounded-2xl overflow-hidden mx-8" style={{ background: 'rgba(0,0,0,0.2)' }}>
                  {bio.map((item, i) => (
                    <div key={item.label} className="flex-1 py-2.5 text-center relative">
                      ...
                      <p style={{ fontSize: '10px', color: 'rgba(255,255,255,0.5)', marginTop: '1px' }}>{item.label}</p>
                    </div>
                  ))}
                </div>
              ) : null;
              ...
              <div className="flex items-center justify-center gap-3 mt-3 flex-wrap">
                {profile?.area ? (
                  <div className="flex items-center gap-1">
                    ...
                    <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.75)' }}>{profile.area}</span>
                  </div>
                ) : null}
      ...
      <div className="px-4 pt-4 pb-24 flex flex-col gap-4">
        ...
        <div className="p-4 rounded-2xl border" style={{ background: cardBg, borderColor }}>
          <div className="flex items-center justify-between mb-3">
            <span style={{ fontSize: '14px', fontWeight: 600, color: textPrimary }}>Win Rate</span>
            <span style={{ fontSize: '22px', fontWeight: 700, color: '#2E7D32' }}>{winRate}%</span>
          </div>
          ...
          <div className="flex justify-between mt-2">
            <span style={{ fontSize: '11px', color: textSecondary }}>{statWins}W · {statMatches - statWins}L/D</span>
            <span style={{ fontSize: '11px', color: textSecondary }}>{statMatches} played</span>
          </div>
        </div>
        ...
        {playerMatches.length > 0 ? (
          <div>
            <h3 style={{ fontSize: '16px', fontWeight: 600, color: textPrimary, marginBottom: '10px' }}>Recent Matches</h3>
                ...
                return (
                  <button key={match.id} onClick={() => navigate(`/app/matches/${match.id}/pre`)}
                    ...
                    <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0"
                      style={{ background: resultColor + '18', color: resultColor, fontSize: '12px', fontWeight: 700 }}>
                      {resultLabel}
                    </div>
                    ...
                    <span style={{ fontSize: '17px', fontWeight: 700, color: resultColor, fontVariantNumeric: 'tabular-nums' }}>
          ...
          <div className="p-6 rounded-2xl border text-center" style={{ background: cardBg, borderColor }}>
            <p style={{ fontSize: '28px', marginBottom: '8px' }}>⚽</p>
            <p style={{ fontSize: '15px', fontWeight: 500, color: textPrimary }}>No matches yet</p>
            <p style={{ fontSize: '13px', color: textSecondary, marginTop: '4px' }}>Join or challenge a team to get started</p>
          </div>
        )}

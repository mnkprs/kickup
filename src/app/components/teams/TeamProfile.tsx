...
function TeamStatsGrid({ statItems, winRate }: { statItems: StatItem[]; winRate: number }) {
          ...
          return (
            <div key={tm.id} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {p.position ? <p style={{ fontSize: '11px', color: textSecondary }}>{p.position}</p> : null}
              </div>
              <div className="flex gap-2">
                <button onClick={() => onAccept(tm.player_id)} disabled={busy}
                ...
                <button onClick={() => onReject(tm.player_id)} disabled={busy}
                  className="w-8 h-8 rounded-full flex items-center justify-center"
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
            ...
            <div className="flex flex-col gap-2">
              {activeMembers.map(tm => {
                const p = tm.profiles;
                if (!p) return null;
                const canRemove = isCaptain && tm.player_id !== userId;
                  ...
                  <div key={tm.id} className="flex items-center gap-3 p-3 rounded-2xl border"
                    ...
                    {isCaptain ? (
                      <div className="flex gap-1">
                        {canMakeCaptain ? (
                          <button onClick={() => onMakeCaptain(tm.player_id)} disabled={busyCaptain}
                        ...
                        {canRemove ? (
                          <button onClick={() => onRemove(tm.player_id)} disabled={busyRemove}
                            className="p-1.5 rounded-xl"
                            style={{ background: '#FFEBEE', border: '1px solid #B3261E' }}
                            title="Remove member">
...
function MatchHistorySection({ completedMatches, teamId, navigate }: {
  ...
  return (
    <div style={{ background: bg, minHeight: '100vh', fontFamily: 'Roboto, sans-serif' }}>
      ...
      <div className="px-4 pt-4 pb-24">
        <TeamStatsGrid statItems={statItems} winRate={winRate} />
        ...
        {completedMatches.length > 0 ? (
          <MatchHistorySection completedMatches={completedMatches} teamId={team.id} navigate={navigate} />
        ) : null}

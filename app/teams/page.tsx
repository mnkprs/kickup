...
import { teams, currentUser, type Team } from "@/lib/mock-data";
...
const filters = ["All", "My Teams", "Open"];

function getTeamRecord(team: Team) {
  return { total: team.wins + team.draws + team.losses, w: team.wins, d: team.draws, l: team.losses };
}
...
export default function TeamsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter === "My Teams") {
      result = result.filter((t) => t.id === currentUser.team_id);
    } else if (activeFilter === "Open") {
      result = result.filter((t) => {
        const memberCount = t.members?.length ?? 0;
        return memberCount < 14;
      });
    }
    ...
    <div className="min-h-dvh bg-background max-w-lg mx-auto relative">
      ...
      <main className="px-5 flex flex-col gap-3 pb-24">
        {filtered.map((team) => {
          ...
          const maxRoster = 14;
          const openSpots = maxRoster - memberCount;
          const isMyTeam = team.id === currentUser.team_id;
          const winRate = record.total > 0 ? Math.round((team.wins / record.total) * 100) : 0;

...
BEGIN
  ...
  DELETE FROM matches WHERE id IN (
    SELECT match_id FROM tournament_matches WHERE tournament_id = p_tournament_id
  );
  ...
  FOR v_i IN 1..8 LOOP
    ...
    VALUES (p_tournament_id, v_teams[v_i], 'approved')
  ...
  INSERT INTO tournament_groups (tournament_id, team_id, group_label) VALUES
    (p_tournament_id, v_teams[1], 'A'), (p_tournament_id, v_teams[2], 'A'),
    (p_tournament_id, v_teams[3], 'A'), (p_tournament_id, v_teams[4], 'A'),
    (p_tournament_id, v_teams[5], 'B'), (p_tournament_id, v_teams[6], 'B'),
    (p_tournament_id, v_teams[7], 'B'), (p_tournament_id, v_teams[8], 'B')
  ...
  FOR v_i IN 1..12 LOOP
    ...
    VALUES (p_tournament_id, v_match_id, 'group', v_group_labels[v_i], v_match_order);
  END LOOP;
  ...
  FOR v_match IN
    ...
    WHERE tm.tournament_id = p_tournament_id AND tm.stage = 'group' ORDER BY tm.match_order
  LOOP
  ...
  LOOP
    ...
    LOOP
      v_team_id2 := v_match.home_team_id;
      ...
      VALUES (v_match.match_id, v_team_id2, v_scorer_id, v_minute);
    END LOOP;
    FOR v_goal IN SELECT * FROM jsonb_array_elements(v_ev->'a')
    LOOP
      v_team_id2 := v_match.away_team_id;
      v_minute := (v_goal->>1)::int;
      v_scorer_id := ('00000000-0000-0000-0000-' || lpad((v_goal->>0)::text, 12, '0'))::uuid;
      INSERT INTO match_events (match_id, team_id, scorer_id, minute)
      VALUES (v_match.match_id, v_team_id2, v_scorer_id, v_minute);
